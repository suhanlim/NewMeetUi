...
const baseStyles = {
  solid:
    "inline-flex justify-center rounded-md py-1 px-4 text-xl font-semibold tracking-tight shadow-sm focus:outline-none",
  outline:
    "inline-flex justify-center rounded-md border py-[calc(theme(spacing.1)-1px)] px-[calc(theme(spacing.4)-1px)] text-xl font-semibold tracking-tight focus:outline-none",
};
...
export function Button<
  ...
  Color extends ColorKey<Variant> = "slate",
>({ variant, color, className, ...props }: ButtonProps<Variant, Color>) {
  variant = variant ?? ("solid" as Variant);
  color = color ?? ("slate" as Color);

  const cls =
    variant in variantStyles && color in variantStyles[variant]
      ? variantStyles[variant][color]
      : undefined;

  className = clsx(baseStyles[variant], cls ?? "", className);
